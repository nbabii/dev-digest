...
export const FEATURE_MODELS: FeatureModelDef[] = [
  ...
  },
  {
    ...
    // Kept in sync with server/src/vendor/shared/contracts/platform.ts and
    // client/src/lib/feature-models.ts (the UI-consumed mirror) — see that
    // file's comment for why this differs from resolveFeatureModel's usual
    // registry-default pattern.
    defaultProvider: 'openrouter',
    defaultModel: 'deepseek/deepseek-v4-flash',
  },
];
...
export const PrMeta = z.object({
  ...
  // Latest COMPLETED run's USD cost (list endpoint only; null/absent when
  // never reviewed or no usage/pricing data).
  cost_usd: z.number().nullish(),
  // Open (undismissed) findings by severity, summed across every completed
  // review for this PR (list endpoint only). See server/specs/findings-counter.md.
  findings: z
    .object({
